...
DX12GraphicsPassEncoder::DX12GraphicsPassEncoder(DX12CommandBuffer const* command)
{
    m_ref_command_buffer = command;
}
...
void DX12GraphicsPassEncoder::bind_vertex_buffers(GPUBufferBinding const* bindings, uint32_t binding_count)
{
    ...
    DX12CommandBuffer const* command_buffer = static_cast<DX12CommandBuffer const*>(m_ref_command_buffer);
    command_buffer->m_command_list->IASetVertexBuffers(0, binding_count, vertex_buffer_views.data());
}
...
void DX12GraphicsPassEncoder::bind_index_buffer(GPUBufferBinding const& binding)
{
    ...
    command_buffer->m_command_list->IASetIndexBuffer(&index_buffer_view);
}
...
void DX12GraphicsPassEncoder::bind_descriptor_set(GPUDescriptorSet const* set)
{
    ...
    command_buffer->reset_root_signature(GPUPipelineType::e_graphics, root_signature->m_root_signature);
    if (descriptor_set->m_cbv_srv_uav_handle != 0)
        command_buffer->m_command_list->SetGraphicsRootDescriptorTable(descriptor_set->m_set_index,
            { command_buffer->m_bound_descriptor_heaps[0]->start_handle.gpu.ptr + descriptor_set->m_cbv_srv_uav_handle });
    else
        command_buffer->m_command_list->SetGraphicsRootDescriptorTable(descriptor_set->m_set_index,
            { command_buffer->m_bound_descriptor_heaps[1]->start_handle.gpu.ptr + descriptor_set->m_sampler_handle });
}
...
void DX12GraphicsPassEncoder::bind_pipeline(GPUGraphicsPipeline const* pipeline)
{
    ...
    command_buffer->reset_root_signature(GPUPipelineType::e_graphics, dx12_root_signature->m_root_signature);
    command_buffer->m_command_list->IASetPrimitiveTopology(dx12_pipeline->m_primitive_topology);
    command_buffer->m_command_list->SetPipelineState(dx12_pipeline->m_pipeline_state);
}
...
void DX12GraphicsPassEncoder::set_viewport(float x, float y, float width, float height, float min_depth, float max_depth)
{
    ...
    DX12CommandBuffer const* command_buffer = static_cast<DX12CommandBuffer const*>(m_ref_command_buffer);
    command_buffer->m_command_list->RSSetViewports(1, &viewport);
}
...
void DX12GraphicsPassEncoder::set_scissor(uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
    ...
    DX12CommandBuffer const* command_buffer = static_cast<DX12CommandBuffer const*>(m_ref_command_buffer);
    command_buffer->m_command_list->RSSetScissorRects(1, &scissor);
}
...
void DX12GraphicsPassEncoder::set_push_constant(GPURootSignature const* root_signature, String const& name, void const* data)
{
    ...
    for (uint32_t i = 0; i < dx12_root_signature->m_push_constant_count; i++)
    {
        ...
        {
            command_buffer->m_command_list->SetGraphicsRoot32BitConstants(dx12_root_signature->m_constant_parameters[i].index,
...
{
    DX12CommandBuffer const* command_buffer = static_cast<DX12CommandBuffer const*>(m_ref_command_buffer);
    command_buffer->m_command_list->DrawInstanced(vertex_count, 1, first_vertex, 0);
}
...
{
    DX12CommandBuffer const* command_buffer = static_cast<DX12CommandBuffer const*>(m_ref_command_buffer);
    command_buffer->m_command_list->DrawInstanced(vertex_count, instance_count, first_vertex, first_instance);
}

void DX12GraphicsPassEncoder::draw_indexed(uint32_t index_count, uint32_t first_index, uint32_t first_vertex)
{
    DX12CommandBuffer const* command_buffer = static_cast<DX12CommandBuffer const*>(m_ref_command_buffer);
    command_buffer->m_command_list->DrawIndexedInstanced(index_count, 1, first_index, static_cast<int>(first_vertex), 0);
}
...
{
    DX12CommandBuffer const* command_buffer = static_cast<DX12CommandBuffer const*>(m_ref_command_buffer);
    command_buffer->m_command_list->DrawIndexedInstanced(index_count, instance_count, first_index, static_cast<int>(first_vertex), first_instance);
}
