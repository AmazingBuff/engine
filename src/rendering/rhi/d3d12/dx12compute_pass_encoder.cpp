...
DX12ComputePassEncoder::DX12ComputePassEncoder(DX12CommandBuffer const* command)
{
...
void DX12ComputePassEncoder::bind_descriptor_set(GPUDescriptorSet const* set)
{
    ...
    command_buffer->m_command_list->SetComputeRootDescriptorTable(descriptor_set->m_set_index, {command_buffer->m_bound_descriptor_heaps[0]->start_handle.gpu.ptr + descriptor_set->m_cbv_srv_uav_handle});
}
...
void DX12ComputePassEncoder::bind_pipeline(GPUComputePipeline const* pipeline)
{
    ...
    command_buffer->reset_root_signature(GPUPipelineType::e_compute, dx12_root_signature->m_root_signature);
    command_buffer->m_command_list->SetPipelineState(dx12_compute_pipeline->m_pipeline_state);
}
...
void DX12ComputePassEncoder::set_push_constant(GPURootSignature const* root_signature, String const& name, void const* data)
{
    ...
    for (uint32_t i = 0; i < dx12_root_signature->m_push_constant_count; i++)
    {
        ...
        {
            command_buffer->m_command_list->SetComputeRoot32BitConstants(dx12_root_signature->m_constant_parameters[i].index,
...
{
    DX12CommandBuffer const* command_buffer = static_cast<DX12CommandBuffer const*>(m_ref_command_buffer);
    command_buffer->m_command_list->Dispatch(x, y, z);
}
