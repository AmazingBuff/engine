...
VKFence::VKFence(GPUDevice const* device) : m_fence(nullptr)
{
    ...
    VK_CHECK_RESULT(vk_device->m_device_table.vkCreateFence(vk_device->m_device, &create_info, &VK_Allocation_Callbacks, &m_fence));
...
VKFence::~VKFence()
{
    ...
    vk_device->m_device_table.vkDestroyFence(vk_device->m_device, m_fence, &VK_Allocation_Callbacks);
}
