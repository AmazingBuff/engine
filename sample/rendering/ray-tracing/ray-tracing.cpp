//
// Created by AmazingBuff on 2025/5/30.
//
...
int main()
{
    ...
    Just just;
    auto func = [&](const SDL_Event& event) {
        just.op(event);
    };

    EventCallbackHandler<uint32_t, std::function<void(const SDL_Event& event)>> back;
    back.register_callback(SDL_EVENT_QUIT, EventUserDataType::e_undefined, func);
